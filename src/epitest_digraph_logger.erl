...
-record(state, {digraph = "/* example: ccomps -x test.dot | dot | gvpack -g | neato -Tpng -n2 -s > test.png */\nstrict digraph epitest {\n concentrate=true;pack=true;rankdir=\"LR\";\n node [color=lightyellow, style=filled];\n"}).
...
handle_event(finished, State) ->
    io:format("~s}~n",[lists:flatten(State#state.digraph)]),
    timer:sleep(1000),
    erlang:halt(),
    {ok, State};
